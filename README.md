...
## 代码可视化（Understand Anything）

本项目使用 [Understand Anything](https://github.com/Egonex-AI/Understand-Anything) 插件生成交互式知识图谱，可视化代码架构和依赖关系。
...
```powershell
# 下载安装脚本
$script = "$env:TEMP\ua-install.ps1"
Invoke-WebRequest -UseBasicParsing -Uri 'https://raw.githubusercontent.com/Egonex-AI/Understand-Anything/main/install.ps1' -OutFile $script

# 安装（指定 opencode 平台）
& $script opencode

# 构建插件核心库
$pluginRoot = "$HOME\.understand-anything-plugin"
pnpm install --dir $pluginRoot
pnpm --filter @understand-anything/core build --dir $pluginRoot
```

安装后重启 CLI / IDE 使 skill 生效。

### 生成知识图谱

在项目根目录运行 `/understand` 命令（或在 opencode 中直接调用 understand skill）：
...
| 文件 | 说明 |
|------|------|
| `.ua/knowledge-graph.json` | 知识图谱（节点 + 边 + 架构层 + 导游路线） |
| `.ua/meta.json` | 分析元数据（git commit、时间戳） |
| `.ua/fingerprints.json` | 结构指纹基线（供增量更新用） |
| `.ua/config.json` | 配置（输出语言等） |
| `.ua/.understandignore` | 排除规则（可编辑） |

后续运行 `/understand` 会增量更新（只重新分析变更文件）。

### 启动可视化面板

#### 方式一：自定义服务器（推荐，Windows 稳定）

项目已内置一个轻量 HTTP 服务器脚本 `.ua/serve-dashboard.cjs`，直接服务已构建的 dashboard 静态文件：

```bash
# 先构建 dashboard（仅需一次）
$pluginRoot = "$HOME\.understand-anything-plugin"
pnpm --filter @understand-anything/dashboard build --dir $pluginRoot

# 启动服务器
node .ua/serve-dashboard.cjs
```

启动后终端输出：

```
🔑  Dashboard URL: http://127.0.0.1:5173/?token=ua-local-dev-token-2026
```

浏览器自动打开，无需额外操作。按 `Ctrl+C` 停止。

#### 方式二：Vite dev server（需网络，可能遇缓存问题）

```bash
$env:GRAPH_DIR = "$PWD"
pnpm --filter @understand-anything/dashboard dev --dir "$HOME\.understand-anything-plugin"
```

### 面板功能

- **结构图谱**：交互式力导向图，节点按架构层着色，可点击查看代码和关系
- **模糊/语义搜索**：按名称或含义搜索节点
- **导游路线**：按依赖顺序生成的架构学习路径（15 步）
- **Diff 影响分析**：运行 `/understand-diff` 查看变更影响范围
- **代码查看器**：点击节点直接查看源码
...
| 命令 | 说明 |
|------|------|
| `/understand` | 分析代码库（首次全量，后续增量） |
| `/understand-dashboard` | 启动可视化面板 |
| `/understand-chat` | 基于图谱提问 |
| `/understand-diff` | 分析当前 git 变更影响 |
| `/understand-explain <file>` | 深入讲解特定文件 |
