...
class QueryEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self._max_tokens = max_tokens  # 单次响应的默认最大输出 token（DeepSeek 默认 8192）
    ...
    def make_call_model(self, model: str | None = None, max_tokens: int | None = None) -> CallModelFn:
        ...
        max_tokens 默认取 self._max_tokens（DeepSeek 为 8192，Anthropic 为 16384），
        由 _build_engine 从 .env 解析后注入。kwargs.pop("max_tokens", ...) 的设计：
        允许 query_loop 在运行时覆盖 max_tokens（如错误恢复时 escalate 到 65536）。
        """
        effective_model = model or self._model
        client = self._client
        default_max = max_tokens if max_tokens is not None else self._max_tokens

        async def call_model(**kwargs: Any) -> AsyncIterator[QueryEvent]:
            # pop 而非 get：避免 max_tokens 同时出现在 kwargs 和显式参数中导致 SDK 报错
            effective_max: Any = kwargs.pop("max_tokens", default_max)
