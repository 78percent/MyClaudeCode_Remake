...
def _load_env() -> dict[str, str]:
    ...
    Supported keys: ANTHROPIC_*, DASHSCOPE_API_KEY, DEEPSEEK_*.
    """
    result: dict[str, str] = {}

    # Load from .env in project root first (lower priority)
    # 显式用 UTF-8 读取，避免 Windows 中文区域默认 cp936 导致中文注释解码失败
...
def _resolve_api_config(env: dict[str, str], model_override: str | None = None) -> dict[str, str]:
    """Resolve the active provider into {api_key, base_url, model, max_tokens}.

    DeepSeek-first: 只要在 .env / 环境变量里填了 DEEPSEEK_API_KEY，整条链路
    就走 DeepSeek 的 Anthropic 兼容接口（https://api.deepseek.com/anthropic）。
    否则回退原生 Anthropic。model_override 供 /model 切换命令指定具体模型。
    """
    ds_key = env.get("DEEPSEEK_API_KEY")
    if ds_key:
        try:
            max_tokens = int(env["DEEPSEEK_MAX_TOKENS"]) if env.get("DEEPSEEK_MAX_TOKENS") else DEEPSEEK_DEFAULT_MAX_TOKENS
        except ValueError:
            max_tokens = DEEPSEEK_DEFAULT_MAX_TOKENS
        return {
            "api_key": ds_key,
            "base_url": env.get("DEEPSEEK_BASE_URL") or DEEPSEEK_BASE_URL,
            "model": model_override or env.get("DEEPSEEK_MODEL") or DEEPSEEK_DEFAULT_MODEL,
...
async def _run_print_mode(prompt: str, model: str) -> None:
    ...
    """
    env = _load_env()
    cfg = _resolve_api_config(env, model_override=model)
    if not cfg["api_key"]:
        console.print("[red]Error: No API key found. Add DEEPSEEK_API_KEY (or ANTHROPIC_API_KEY) to .env or environment.[/]")
        sys.exit(1)

    client = create_client(api_key=cfg["api_key"], base_url=cfg["base_url"] or None)
    cwd = str(Path.cwd())

    engine = _build_engine(client, cfg["model"], cwd, is_interactive=False, max_tokens=cfg["max_tokens"])
...
async def _run_repl(model: str, resume_id: str | None = None) -> None:
    ...
    engine = _build_engine(client, cfg["model"], cwd, max_tokens=cfg["max_tokens"])
    ...
    while True:
        ...
        if user_input.strip().startswith("/"):
            ...
            if result == "__CLEAR__":
                ...
                # 切换 provider: 百炼模型走 DASHSCOPE_API_KEY，DeepSeek/Anthropic 走 _resolve_api_config
                ...
                if is_dashscope_model(new_model):
                    ...
                    # DeepSeek 或 Anthropic：按 .env 配置解析
                    _cfg = _resolve_api_config(env, model_override=new_model)
                    if not _cfg["api_key"]:
                        console.print("[red]Error: no API key for this model. Add it to .env or environment.[/]")
                        continue
                    engine._client = create_client(api_key=_cfg["api_key"], base_url=_cfg["base_url"] or None)  # type: ignore[assignment]
...
@click.option("--model", default=None, help="Model to use (defaults to DEEPSEEK_MODEL from .env)")
...
def main(
    ...
    # 解析最终模型：命令行 --model 优先，否则读 .env 的 DEEPSEEK_MODEL
    resolved_model = _resolve_api_config(_load_env(), model_override=model)["model"]
    ...
    if print_mode:
        ...
        if not prompt:
            ...
            sys.exit(1)
        asyncio.run(_run_print_mode(prompt, resolved_model))
    else:
        asyncio.run(_run_repl(resolved_model, resume_id=resume_id))
