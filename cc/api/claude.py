...
async def stream_response(
    ...
    *,
    ...
    try:
        ...
        # httpx 层流式传输中断——服务端在输出到一半时断开连接。
        # 典型触发：DeepSeek 的 Anthropic 兼容端点在流式输出 tool_use 块时
        # 偶发掐断 TCP 连接（peer closed connection / incomplete chunked read）。
        # SDK 的流式迭代不会把这类错误包装成 APIConnectionError，必须显式捕获，
        # 否则会冒泡到 REPL 顶层导致整个程序崩溃。
        yield ErrorEvent(message=f"Stream interrupted: {e}", is_recoverable=True)
        return
